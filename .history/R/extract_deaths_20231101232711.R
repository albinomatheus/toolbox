...
#' @export
#'
extract_deaths <- function(data, location, sex, year1, year2, omega = 100, trunc = 70) {
  ...
  df_list <- years_between %>%
    purrr::map(~ d_0 |>
                 ...
                 dplyr::distinct() |>
                 dplyr::mutate(idadeL = ifelse(idadeL >= trunc, trunc, idadeL)) %>%
  ...
  full_abridged <- df |>
    tidyr::pivot_longer(-year, names_to = "age", values_to = "deaths") |>
    dplyr::mutate(
      age = ifelse(as.integer(age) >= 85, 85, calcAgeAbr(as.integer(age))),
      year = as.numeric(year) + years_between[1] - 1) |>
    group_by(year, age) |>
  ...
  is.sequential <- \(x) {
    if (all(diff(x) == diff(x)[1])) {
      ...
      return(print("Atenção: a série não representa a soma de todo o período"))
    }
  }
