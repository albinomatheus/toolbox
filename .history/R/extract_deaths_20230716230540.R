...
extract_deaths <- function(data, location, sex, year1, year2, omega = 100) {
  ...
  df_list <- years_between %>%
    purrr::map(~ d_0 |> 
                 ...
                 dplyr::reframe(age = calcAgeAbr(as.integer(idadeL)),
                   obitos = sum(obitos),
                   width = case_when(idadeU - idadeL == 1 ~ 1,
                                     TRUE ~ idadeU - idadeL + 1)) |> 
                 dplyr::distinct() |> 
                 mutate(idadeL = ifelse(idadeL >= 60, 60, idadeL)) %>%
                 group_by(idadeL) %>%
                 summarise(obitos = sum(obitos), width = sum(width)) %>%
                 mutate(age = idadeL,
                        idadeU = idadeL + width) %>%
                 select(idadeL, idadeU, age, obitos, width)
               )
